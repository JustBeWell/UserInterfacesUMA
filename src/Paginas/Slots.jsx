...
function Slots() {
  ...
  const generatedSlots1 = Array.from(["🍒", "🍋", "🍊","7️⃣","BAR", "🍉", "🍇", "🔔", "⭐", "💎"]);
  const generatedSlots2 = Array.from(["🍒", "🍋", "🍊","7️⃣","BAR", "🍉", "🍇", "🔔", "⭐", "💎"]);
  const generatedSlots3 = Array.from(["🍒", "🍋", "🍊","7️⃣","BAR", "🍉", "🍇", "🔔", "⭐", "💎"]);
  ...
  const spinParada = async(generatedSlots, setColumn, currentIndex) =>{
    ...
    for (let i = 0; i < 40; i++){
      ...
      const speed = i < 10 ? velocidadInicial + ((velocidadMedia - velocidadInicial) * i) / 10 + randomFactor
      : i < 25 ? velocidadMedia + ((velocidadFinal - velocidadMedia) * (i - 10)) / 15 + randomFactor
      : velocidadFinal + ((1000 - velocidadFinal) * (i - 25)) / 15 + randomFactor;
  ...
  const stopSlots = async () => {
    ...
    const finalSlots = [generatedSlots1[(currentIndex1.current+1) % generatedSlots1.length], 
    generatedSlots2[(currentIndex2.current+1) % generatedSlots2.length], generatedSlots3[(currentIndex3.current+1) % generatedSlots3.length]];
